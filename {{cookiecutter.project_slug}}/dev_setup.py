...
def check_dependencies() -> bool:
    ...
    for dep in dependencies:
        try:
            subprocess.run(
                ["which", dep], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
...
def install_python_dependencies() -> bool:
    ...
    try:
        subprocess.run(
            ["poetry", "--version"], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
...
def setup_development_environment(args: argparse.Namespace) -> bool:
    ...
    if not args.skip_dependency_check and not check_dependencies():
        if not args.force:
            print_error(
                "Missing dependencies. Install them and try again, or use --force to continue anyway."
            )
            return False
    ...
    if not args.skip_python_deps and not install_python_dependencies():
        if not args.force:
            print_error(
                "Failed to install Python dependencies. Fix the issues and try again, or use --force to continue."
            )
            return False
    ...
    if not args.skip_git_hooks and not setup_git_hooks():
        if not args.force:
            print_error(
                "Failed to set up Git hooks. Fix the issues and try again, or use --force to continue."
            )
            return False
    ...
    if not args.skip_env_files and not create_env_files():
        if not args.force:
            print_error(
                "Failed to create environment files. Fix the issues and try again, or use --force to continue."
            )
            return False
    ...
    if not args.skip_vscode and not configure_vscode():
        if not args.force:
            print_error(
                "Failed to configure VSCode. Fix the issues and try again, or use --force to continue."
            )
            return False
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Set up development environment for Process project"
    )
    ...
    parser.add_argument(
        "--skip-python-deps", action="store_true", help="Skip Python dependency installation"
    )
    parser.add_argument("--skip-git-hooks", action="store_true", help="Skip Git hook setup")
    parser.add_argument(
        "--skip-env-files", action="store_true", help="Skip environment file creation"
    )
