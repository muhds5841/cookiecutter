...
from core.monitoring import MetricsCollector, HealthCheck, create_health_endpoint
...
class FTPServer:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.process_extensions = tuple(os.environ.get("FTP_PROCESS_EXTENSIONS", ".txt,.md").split(','))
        ...
        if self.enable_metrics:
            self.metrics = MetricsCollector(
                service_name="ftp",
                metrics_port=int(os.environ.get("FTP_METRICS_PORT", "9102"))
            )
            self.metrics.register_counter("ftp_connections_total", "Total number of FTP connections")
            self.metrics.register_counter("ftp_files_received_total", "Total number of files received")
            self.metrics.register_counter("ftp_files_sent_total", "Total number of files sent")
            self.metrics.register_counter("ftp_files_processed_total", "Total number of files processed")
            self.metrics.register_counter("ftp_processing_errors_total", "Total number of processing errors")
            self.metrics.register_gauge("ftp_active_connections", "Number of active FTP connections")
