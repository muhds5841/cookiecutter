...
class IMAPClient:
    ...
    def __init__(
        ...
        self.port = port or int(
            os.environ.get("IMAP_SSL_PORT" if use_ssl else "IMAP_PORT", "993" if use_ssl else "143")
        )
        ...
        self.use_ssl = (
            ...
            else os.environ.get("IMAP_USE_SSL", "false").lower() == "true"
        )
        ...
        if self.enable_metrics:
            ...
            self.metrics.register_counter(
                "imap_messages_retrieved_total", "Total number of messages retrieved"
            )
            self.metrics.register_counter(
                "imap_attachments_retrieved_total", "Total number of attachments retrieved"
            )
            self.metrics.register_gauge(
                "imap_connection_time_seconds", "Time to connect to IMAP server"
            )
            self.metrics.register_histogram(
                "imap_message_size_bytes", "Size of retrieved messages in bytes"
            )
    ...
    def _health_check_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "healthy",
                "details": {"connection_time": elapsed_time, "server": f"{self.host}:{self.port}"},
            }
    ...
    def connect(self):
        ...
        try:
            ...
            elapsed_time = time.time() - start_time
            self.logger.info(
                f"Połączono z serwerem IMAP {self.host}:{self.port} w {elapsed_time:.2f}s"
            )
    ...
    def get_messages(
        self, folder: str = None, limit: int = 10, criteria: str = "ALL"
    ) -> List[Dict[str, Any]]:
