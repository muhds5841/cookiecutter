...
class WebSocketServer:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.enable_compression = (
            os.environ.get("WEBSOCKET_ENABLE_COMPRESSION", "true").lower() == "true"
        )
    ...
    async def register(self, websocket: WebSocketServerProtocol):
        ...
        self.logger.info(
            f"New connection registered. Active connections: {len(self.active_connections)}"
        )
        ...
        try:
            ...
            if "text" not in payload:
                await self.send_error(
                    websocket, "Missing required field 'text'", payload.get("request_id")
                )
                return
    ...
    async def handle_resources_request(
        self, websocket: WebSocketServerProtocol, payload: Dict[str, Any]
    ):
        ...
        try:
            ...
            # Prepare response
            response = {"request_id": request_id, "resources": resources}
    ...
    async def send_error(
        self, websocket: WebSocketServerProtocol, error_message: str, request_id: str = ""
    ):
        """Send an error message."""
        error_response = {"request_id": request_id, "error": error_message}
