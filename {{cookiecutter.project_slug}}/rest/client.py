...
class ProcessClient:
    ...
    def synthesize(
        ...
        try:
            # Wyślij żądanie POST
            response = requests.post(f"{self.base_url}/api/v1/synthesize", json=payload)
    ...
    def download_audio(self, audio_id: str, output_path: Optional[Union[str, Path]] = None) -> str:
        ...
        try:
            ...
            response = requests.get(
                f"{self.base_url}/api/v1/audio/{audio_id}", stream=True  # Pobieranie strumieniowe
            )
...
def main():
    ...
    configure_logging(
        level=config.get("REST_CLIENT_LOG_LEVEL", "info"), component_name="rest_client"
    )
